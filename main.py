...
def update_graph(gender_value, senior_citizen_value, 
                 phone_service_value, internet_service_value, 
                 contract_value, payment_method_value):

    data_copy = data.copy()

    if gender_value is not None:
        gender_filter = GetSidebarElementsValue.get_sidebar_value(
            gender_value, "gender")
        data_copy = data_copy[data_copy['gender'] == gender_filter]

    if senior_citizen_value is not None:
        senior_filter = GetSidebarElementsValue.get_sidebar_value(
            senior_citizen_value, "senior_citizen")
        data_copy = data_copy[data_copy['SeniorCitizen'] == senior_filter]

    if phone_service_value is not None:
        phone_filter = GetSidebarElementsValue.get_sidebar_value(
            phone_service_value, "phone_service")
        data_copy = data_copy[data_copy['PhoneService'] == phone_filter]

    if internet_service_value is not None:
        internet_filter = GetSidebarElementsValue.get_sidebar_value(
            internet_service_value, "internet_service")
        data_copy = data_copy[data_copy['InternetService'] == internet_filter]

    if contract_value is not None:
        contract_filter = GetSidebarElementsValue.get_sidebar_value(
            contract_value, "contract")
        data_copy = data_copy[data_copy['Contract'] == contract_filter]

    if payment_method_value is not None:
        payment_filter = GetSidebarElementsValue.get_sidebar_value(
            payment_method_value, "payment_method")
        data_copy = data_copy[
            data_copy['PaymentMethod'] == payment_filter]

    figure1 = px.scatter(
        data_copy, x='MonthlyCharges', y='TotalCharges', color='gender',
    ...
    figure3 = px.histogram(
        data_copy, x='MultipleLines', color='MultipleLines', 
    ...
    streaming_tv_avg_charges = data_copy.groupby(
        'StreamingTV')['MonthlyCharges'].mean().reset_index()
    figure4 = px.bar(
        streaming_tv_avg_charges, x='StreamingTV', y='MonthlyCharges',
    ...
    figure7 = px.pie(
        names=online_security_counts.index, 
        values=online_security_counts.values,
